...
async function init() {
    ...
    app.post('/release', (req, res) => releaseDeviceRoute(req, res));
...
async function releaseDeviceRoute(req, res) {
    try {
        await query.initialize();
        const result = await query.releaseDevice(req.body.imei, req.body.manufacturer, req.body.name, req.body.price, req.body.description);
        res.status(200).send(result);
    } catch (error) {
