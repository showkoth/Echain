...
async function initialize() {
    try {
        ...
        const identity = await wallet.get('manufacturer');
        if (!identity) {
            console.log('An identity for the user "manufacturer" does not exist in the wallet');
...
async function viewUnsold() {
    try {
        ...
        await gateway.connect(ccp, { wallet, identity: 'manufacturer', discovery: { enabled: true, asLocalhost: true } });
...
async function releaseDevice(imei, manufacturer, name, price, description) {
    try {
        // create a new gateway for connecting to our peer node.
        const gateway = new Gateway();
        await gateway.connect(ccp, { wallet, identity: 'manufacturer', discovery: { enabled: true, asLocalhost: true } });
        ...
        // evaluate the specified transaction.
        const result = await contract.submitTransaction('releaseDevice', imei, manufacturer, name, price, description, false, "None", "None", "None");
        console.log(`Transaction has been evaluated. Result is: ${result.toString()}`);
...
exports.releaseDevice = releaseDevice;
