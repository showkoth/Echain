...
async function initialize() {
    try {
        ...
        const identity = await wallet.get('consumer');
        if (!identity) {
            console.log('An identity for the user "consumer" does not exist in the wallet');
...
async function viewUnsold() {
    try {
        ...
        await gateway.connect(ccp, { wallet, identity: 'consumer', discovery: { enabled: true, asLocalhost: true } });
...
async function distributeDevice(imei, distributor) {
    try {
        ...
        const result = await contract.submitTransaction('buyDevice', imei, distributor);
        console.log(`Transaction has been evaluated. Result is: ${result.toString()}`);
