...
async function initialize() {
    try {
        ...
        const identity = await wallet.get('assembler');
        if (!identity) {
            console.log('An identity for the user "assembler" does not exist in the wallet');
...
async function viewUnsold() {
    try {
        ...
        await gateway.connect(ccp, { wallet, identity: 'assembler', discovery: { enabled: true, asLocalhost: true } });
...
async function assembleDevice(imei, assembler, qcpassdate) {
    try {
        // create a new gateway for connecting to our peer node.
        const gateway = new Gateway();
        await gateway.connect(ccp, { wallet, identity: 'assembler', discovery: { enabled: true, asLocalhost: true } });
        ...
        // evaluate the specified transaction.
        var shipmentStatus = 'Device is now at assembler end'
        const result = await contract.submitTransaction('assembleDevice', imei, assembler);
        console.log(`Transaction has been evaluated. Result is: ${result.toString()}`);
...
exports.assembleDevice = assembleDevice;
